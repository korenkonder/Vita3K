...
static void create_fragment_inputs(CodeWriter &writer, ShaderVariables &variables, const FeatureState &features, TranslationState &translation_state, const SceGxmProgram &program) {
    ...
    if (program.is_frag_color_used()) {
        if (features.direct_fragcolor) {
            ...
            variables.should_gen_pack_unpack[ShaderVariables::GEN_PACK_4XU16] = true;
                       ...
                       [&](const AttributeInputSource &s) {
                           if (input.bank == RegisterBank::SPECIAL) {
                               ...
                               case SCE_GXM_PARAMETER_SEMANTIC_INDEX:
                                   ...
                                   store_info.data_type = DataType::F32;
                               ...
                               default:
                                   store_info.data_type = input.type;
                               }
    ...
    if (program.is_vertex() && (in_fcount_allocated == 0) && (program.primary_reg_count != 0)) {
        ...
        if (hints) {
            ...
            for (std::size_t i = 0; i < attributes->size(); i++) {
                VarToReg store_info;
                store_info.location = attributes->at(i).regIndex / 4;
                store_info.offset = attributes->at(i).regIndex - store_info.location * 4;
                store_info.comp_count = attributes->at(i).componentCount;
                store_info.comp_count = (attributes->at(i).componentCount + 3) / 4 * 4;
                store_info.data_type = DataType::F32;
...
static std::string convert_gxp_to_glsl_impl(const SceGxmProgram &program, const std::string &shader_hash, const FeatureState &features,
    TranslationState &translation_state, const Hints *hints, bool force_shader_debug,
...
GeneratedShader convert_gxp_to_glsl(const SceGxmProgram &program, const std::string &shader_hash, const FeatureState &features,
    const Hints &hints, bool maskupdate, bool force_shader_debug,
    std::function<bool(const std::string &ext, const std::string &dump)> dumper) {
