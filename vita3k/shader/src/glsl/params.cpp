...
#undef GEN_CONSTANT
        ...
        if ((op.type == DataType::F32) || (op.type == DataType::UINT32) || (op.type == DataType::INT32)) {
            auto get_f32_from_bank = [&](const int num) -> std::string {
                ...
                std::string result = fmt::format("{}", std::bit_cast<float>(value));
...
std::string ShaderVariables::convert_to_float(std::string src, const std::int32_t comp_count, DataType type, bool normal) {
    ...
    if (normal) {
        if (comp_count == 1) {
            switch (type) {
            case DataType::UINT8:
                src = fmt::format("float({}) / 255.0", src);
                break;

            case DataType::UINT16:
                src = fmt::format("float({}) / 65535.0", src);
                break;

            case DataType::UINT32:
                src = fmt::format("float({}) / 4294967295.0", src);
                break;

            case DataType::INT8:
                src = fmt::format("(({} < 0) ? float({}) / 128.0, float({}) / 127.0)", src, src, src);
                break;

            case DataType::INT16:
                src = fmt::format("(({} < 0) ? float({}) / 32768.0, float({}) / 32767.0)", src, src, src);
                break;

            case DataType::INT32:
                src = fmt::format("(({} < 0) ? float({}) / 2147483648.0, float({}) / 2147483647.0)", src, src, src);
                break;

            case DataType::C10:
                // signed 10-bit
                src = fmt::format("(({} < 0) ? float({}) / 512.0, float({}) / 511.0)", src, src, src);
                break;
            ...
            switch (type) {
            case DataType::UINT8:
                src = fmt::format("vec{}({}) / 255.0", comp_count, src);
                break;

            case DataType::UINT16:
                src = fmt::format("vec{}({}) / 65535.0", comp_count, src);
                break;

            case DataType::UINT32:
                src = fmt::format("vec{}({}) / 4294967295.0", comp_count, src);
                break;

            case DataType::INT8:
                src = fmt::format("mix(vec{}({}) / 128.0, vec{}({}) / 127.0, greaterThanEqual({}, vec{}(0.0)))",
                    comp_count, src, comp_count, src, src, comp_count);
                break;

            case DataType::INT16:
                src = fmt::format("mix(vec{}({}) / 32768.0, vec{}({}) / 32767.0, greaterThanEqual({}, vec{}(0.0)))",
                    comp_count, src, comp_count, src, src, comp_count);
                break;

            case DataType::INT32:
                src = fmt::format("mix(vec{}({}) / 2147483648.0, vec{}({}) / 2147483647.0, greaterThanEqual({}, vec{}(0.0)))",
                    comp_count, src, comp_count, src, src, comp_count);
                break;

            case DataType::C10:
                // signed 10-bit
                src = fmt::format("mix(vec{}({}) / 512.0, vec{}({}) / 511.0, greaterThanEqual({}, vec{}(0.0)))",
                    comp_count, src, comp_count, src, src, comp_count);
                break;
...
std::string ShaderVariables::convert_to_int(std::string src, const std::int32_t comp_count, DataType type, bool normal) {
    ...
    std::string dest_dt_type = is_unsigned_integer_data_type(type) ? "uint" : "int";
    if (normal) {
        if (comp_count == 1) {
            switch (type) {
            case DataType::UINT8:
                src = fmt::format("{} * 255.0", src);
                break;

            case DataType::UINT16:
                src = fmt::format("{} * 65535.0", src);
                break;

            case DataType::UINT32:
                src = fmt::format("{} * 4294967295.0", src);
                break;

            case DataType::INT8:
                src = fmt::format("({} < 0) ? ({} * 128.0) : ({} * 127.0)", src, src, src);
                break;

            case DataType::INT16:
                src = fmt::format("({} < 0) ? ({} * 32768.0) : ({} * 32767.0)", src, src, src);
                break;

            case DataType::INT32:
                src = fmt::format("({} < 0) ? ({} * 2147483648.0) : ({} * 2147483647.0)", src, src, src);
                break;

            case DataType::C10:
                // signed 10-bit
                src = fmt::format("({} < 0) ? ({} * 512.0) : ({} * 511.0)", src, src, src);
                break;
            ...
            src = dest_dt_type + "(round(" + src + "))";
        } else {
            switch (type) {
            case DataType::UINT8:
                src = fmt::format("{} * vec{}(255.0)", src, comp_count);
                break;

            case DataType::UINT16:
                src = fmt::format("{} * vec{}(65535.0)", src, comp_count);
                break;

            case DataType::UINT32:
                src = fmt::format("{} * vec{}(4294967295.0)", src, comp_count);
                break;

            case DataType::INT8:
                src = fmt::format("mix({} * 128.0, {} * 127.0, greaterThanEqual({}, vec{}(0.0)))", src, src, src, comp_count);
                break;

            case DataType::INT16:
                src = fmt::format("mix({} * 32768.0, {} * 32767.0, greaterThanEqual({}, vec{}(0.0)))", src, src, src, comp_count);
                break;

            case DataType::INT32:
                src = fmt::format("mix({} * 2147483648.0, {} * 2147483647.0, greaterThanEqual({}, vec{}(0.0)))", src, src, src, comp_count);
                break;

            case DataType::C10:
                // signed 10-bit
                src = fmt::format("mix({} * 512.0, {} * 511.0, greaterThanEqual({}, vec{}(0.0)))", src, src, src, comp_count);
                break;
            ...
            src = fmt::format("{}vec{}(round({{))", dest_dt_type[0], comp_count, src);
        }
    } else {
        if (comp_count == 1) {
            src = dest_dt_type + "(" + src + ")";
        } else {
