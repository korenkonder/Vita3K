...
bool USSETranslatorVisitorGLSL::vbw(
    ...
    if (immediate) {
        ...
        if (src2_rot > 0) {
            if (decoded_inst.opr.src2.type == DataType::UINT16) {
                src2_rot &= 15;
        ...
        if (decoded_inst.opr.src2.type == DataType::UINT16)
            value &= 0xFFFF;
        src2 = fmt::format("{}", value);
        src2_constant = value;
        is_src2_const = true;
    } else {
        src2 = variables.load(decoded_inst.opr.src2, 0b0001, src2_repeat_offset);

        if (src2_rot) {
            if (decoded_inst.opr.src2.type == DataType::UINT16)
                src2_rot &= 15;
            const uint32_t right_shift = decoded_inst.opr.src2.type == DataType::UINT16 ? (16 - src2_rot) : (32 - src2_rot);

            src2 = fmt::format("(({} << {}) | ({} >> {}))", src2, src2_rot, src2, right_shift);
    ...
    case Opcode::ROL:
        ...
        return false; // TODO: GLSL doesn't seem to have a rotate left operation!
    ...
    auto const_val = is_src2_const ? src2_constant : 1; // default value is intentionally non zero
    if (((decoded_inst.opcode == Opcode::OR || decoded_inst.opcode == Opcode::XOR) && is_src2_const && const_val == 0)
        || (decoded_inst.opcode == Opcode::AND && is_src2_const && const_val == std::numeric_limits<decltype(const_val)>::max())) {
        result = variables.load(decoded_inst.opr.src1, 0b0001, src1_repeat_offset);
        if (result.empty()) {
        ...
        result = fmt::format("({}) {} ({})", src1, operator_str, src2);
        if (is_float_data_type(decoded_inst.opr.src2.type)) {
            result = std::string("uintBitsToFloat(") + src2 + ")";
        }
    }
    ...
    variables.store(decoded_inst.opr.dest, result, 0b0001, dest_repeat_offset);
    END_REPEAT()
...
bool USSETranslatorVisitorGLSL::i32mad(
    ...
    std::string vsrc0 = variables.load(decoded_inst.opr.src0, decoded_source_mask, src0_repeat_offset);
    std::string vsrc1 = variables.load(decoded_inst.opr.src1, decoded_source_mask_2, src1_repeat_offset);
    std::string vsrc2 = variables.load(decoded_inst.opr.src2, decoded_source_mask_3, src2_repeat_offset);
