...
std::string USSETranslatorVisitorGLSL::vtst_impl(Instruction inst, ExtPredicate pred, int zero_test, int sign_test, Imm4 load_mask, bool mask) {
    ...
    if (is_sub_opcode(inst.opcode)) {
        ...
        lhs = do_alu_op(inst, load_mask, mask ? load_mask : 0b1);
        if (num_comp_count > 1) {
...
bool USSETranslatorVisitorGLSL::vtstmsk(
    ...
    const bool is_vdp = (decoded_inst.opcode == Opcode::VDP || decoded_inst.opcode == Opcode::VF16DP);
    const bool output_4 = (alu_sel == 0 && tst_mask_type == 2);

    std::string pred_result = vtst_impl(decoded_inst, pred, zero_test, sign_test, 0b1111, true);
    ...
    default: {
        ...
        case DataType::UINT32:
            ...
            break;
        default:
            LOG_ERROR("Unexpected type {}", static_cast<int>(decoded_inst.opr.dest.type));
            break;
        }
        zeros = is_signed ? "ivec4(0)" : "uvec4(0u)";
        ones = fmt::format("{}vec4({:#X}{})", is_signed ? "i" : "u", ones_val, is_signed ? "" : "u");
        break;
    }
    }

    pred_result = fmt::format("mix({}, {}, {})", zeros, ones, pred_result);

    // These can't be stored to Predicate registers obiviously
    variables.store(decoded_inst.opr.dest, pred_result, 0b1111, 0);
