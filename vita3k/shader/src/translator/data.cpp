...
bool USSETranslatorVisitor::vmov(
    ...
    if (move_data_type == DataType::F16 || move_data_type == DataType::F32)
        ...
        decoded_dest_mask = 0b1;

    // Velocity uses a vec4 table, non-extended, so i assumes type=vec4, extended=false
    decoded_inst.opr.src1.swizzle = is_double_regs ? decode_vec34_swizzle(src0_swiz, false, 2) : (Swizzle4 SWIZZLE_CHANNEL_4_DEFAULT);
    ...
    if (is_conditional) {
        ...
        decoded_inst.opr.src0.type = is_u8_conditional ? DataType::UINT8 : move_data_type;
...
bool USSETranslatorVisitor::vldst(
    ...
    case 1:
        ...
        break;
    case 2:
        decoded_inst.opcode = Opcode::STR;
        break;
    default:
    ...
    if (is_store) {
        decoded_inst.opr.src2.type = type_to_ldst;
        decoded_inst.opr.dest = decoded_inst.opr.src2;
    } else {
        ...
        if (m_features.support_memory_mapping)
            decoded_inst.opr.dest.type = type_to_ldst;
        else
    ...
    const int dest_offset = is_store ? src2_repeat_offset : 0;
    ...
    std::string disasm_str = fmt::format("{:016x}: {}{}", m_instr, disasm::e_predicate_str(pred), disasm::opcode_str(decoded_inst.opcode));
    LOG_DISASM("{} {} ({} + {} + {}) [{} bytes]", disasm_str, disasm::operand_to_str(decoded_inst.opr.dest, 0b1, dest_offset),
        disasm::operand_to_str(decoded_inst.opr.src0, 0b1, src0_offset),
        disasm::operand_to_str(decoded_inst.opr.src1, 0b1, src1_offset), is_store ? "0" : disasm::operand_to_str(decoded_inst.opr.src2, 0b1, src2_offset), current_bytes_to_fetch);
