...
static void generate_shader_body(spv::Builder &b, const SpirvShaderParameters &parameters, const SceGxmProgram &program,
    ...
    const NonDependentTextureQueryCallInfos &texture_queries, spv::Function *spv_func_main, std::vector<spv::Id> &interfaces) {
    // Do texture queries
    usse::convert_gxp_usse_to_spirv(b, program, features, parameters, utils, begin_hook_func, end_hook_func, texture_queries, spv_func_main, interfaces);
}
...
static SpirvCode convert_gxp_to_spirv_impl(const SceGxmProgram &program, const std::string &shader_hash, const FeatureState &features, TranslationState &translation_state, bool force_shader_debug, std::function<bool(const std::string &ext, const std::string &dump)> dumper) {
    ...
    if (!translation_state.is_maskupdate) {
        ...
        generate_shader_body(b, parameters, program, features, utils, begin_hook_func, end_hook_func, texture_queries, spv_func_main, translation_state.interfaces);
    } else {
...
GeneratedShader convert_gxp_to_spirv(const SceGxmProgram &program, const std::string &shader_hash, const FeatureState &features,
    const Target target, const Hints &hints, bool maskupdate, bool force_shader_debug,
