...
USSERecompilerSpirv::USSERecompilerSpirv(spv::Builder &b, const SceGxmProgram &program, const FeatureState &features, const SpirvShaderParameters &parameters,
    utils::SpirvUtilFunctions &utils, const NonDependentTextureQueryCallInfos &queries)
    : b(b) {
    visitor = std::make_unique<USSETranslatorVisitorSpirv>(b, *this, program, features, utils, cur_instr, parameters, queries, false);
}
...
spv::Function *USSERecompilerSpirv::compile_program_function() {
    ...
    spv::Function *ret_func = b.makeFunctionEntry(
        spv::NoPrecision, b.makeVoidType(), sub_name.c_str(), {}, {},
        {}, &new_sub_block);
...
void convert_gxp_usse_to_spirv(spv::Builder &b, const SceGxmProgram &program, const FeatureState &features, const SpirvShaderParameters &parameters, utils::SpirvUtilFunctions &utils,
    spv::Function *begin_hook_func, spv::Function *end_hook_func, const NonDependentTextureQueryCallInfos &queries, spv::Function *spv_func_main, std::vector<spv::Id> &interfaces) {
    ...
    // TODO: Reuse this
    usse::USSERecompilerSpirv recomp(b, program, features, parameters, utils, queries);
    ...
    if (recomp.get_spirv_translator_visitor()->frag_depth_id != 0) {
        interfaces.push_back(recomp.get_spirv_translator_visitor()->frag_depth_id);
