...
spv::Id shader::usse::USSETranslatorVisitorSpirv::do_fetch_texture(const spv::Id tex, int texture_index, const int dim, const Coord &coord, const DataType dest_type, const int lod_mode,
    const spv::Id extra1, const spv::Id extra2, const int gather4_comp) {
...
void shader::usse::USSETranslatorVisitorSpirv::do_texture_queries(const NonDependentTextureQueryCallInfos &texture_queries) {
    Operand store_op;
...
bool USSETranslatorVisitorSpirv::smp(
    ...
    // Load the coord
    spv::Id coords = load(decoded_inst.opr.src0, coord_mask);
    ...
    if (sb_mode == 2) {
        ...
        if (lod_mode != 0) {
            ...
            switch (lod_mode) {
            case 2:
                extra1 = load(decoded_inst.opr.src2, 0b1);
            ...
            case 3:
                switch (dim) {
                case 2:
                    extra1 = load(decoded_inst.opr.src2, 0b0011);
                    extra2 = load(decoded_inst.opr.src2, 0b1100);
                    break;
                case 3:
                    extra1 = load(decoded_inst.opr.src2, 0b0111);
                    extra2 = load(decoded_inst.opr.src2, 0b0111, 1);
                }
                break;
        ...
        if (is_texture_buffer_load) {
            ...
            if (texture_index != 0) {
                ...
                for (size_t s = 0; s < samplers.size(); s++) {
                    ...
                    spv::Id result = do_fetch_texture(m_b.createLoad(smp->id, spv::NoPrecision), smp->index, dim, { coords, static_cast<int>(DataType::F32) }, decoded_inst.opr.dest.type, lod_mode, extra1);
                    const Imm4 dest_mask = (1U << smp->component_count) - 1;
                    store(decoded_inst.opr.dest, result, dest_mask);
                ...
                spv::Id result = do_fetch_texture(m_b.createLoad(smp->id, spv::NoPrecision), sampler.index, dim, { coords, static_cast<int>(DataType::F32) }, decoded_inst.opr.dest.type, lod_mode, extra1);
                const Imm4 dest_mask = (1U << smp->component_count) - 1;
                store(decoded_inst.opr.dest, result, dest_mask);
            } else {
            ...
            spv::Id result = do_fetch_texture(image_sampler, sampler.index, dim, { coords, static_cast<int>(DataType::F32) }, decoded_inst.opr.dest.type, lod_mode, extra1, extra2);
            ...
            for (int comp = 0; comp < sampler.component_count; comp++) {
                g4_comps.push_back(do_fetch_texture(image_sampler, sampler.index, dim, { coords, static_cast<int>(DataType::F32) }, decoded_inst.opr.dest.type, lod_mode, extra1, extra2, comp));
            }

            if (sampler.component_count == 1) {
                // easy, no need to do all this reordering
                store(decoded_inst.opr.dest, g4_comps[0], 0b1111);
                ...
                for (size_t idx = 0; idx < comps_alone.size(); idx += 4) {
                    ...
                    store(decoded_inst.opr.dest, comp_packed, 0b1111);
            ...
            if (sb_mode == 3) {
                ...
                store(decoded_inst.opr.dest, coeffs, 0b1111);
            }
        }
    }
