...
bool USSETranslatorVisitorSpirv::vldst(
    ...
    if (m_features.support_memory_mapping) {
        utils::buffer_address_access(m_b, m_spirv_params, m_util_funcs, m_features, decoded_inst.opr.dest, to_store_offset, base, get_data_type_size(type_to_ldst), total_number_to_fetch, m_program.is_fragment(), -1, is_store);
    } else {
        if (is_store) {
